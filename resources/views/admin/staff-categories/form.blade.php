...
@php $isEdit = isset($category) && $category !== null; @endphp
@section('title', $isEdit ? 'Edit Category' : 'Add Category')
@section('admin_nav_staff_categories', 'active')
@section('admin_page_title', $isEdit ? 'Edit Staff Category' : 'Add Staff Category')
...
<div class="d-flex align-items-center gap-3 mb-4">
    ...
    <div>
        <h5 class="fw-semibold mb-0" style="font-size:18px;">{{ $isEdit ? 'Edit Category' : 'Add New Category' }}</h5>
        <p class="mb-0" style="font-size:13px;color:var(--sb-muted);">
            {{ $isEdit ? 'Update category details below.' : 'Create a new staff category.' }}
        </p>
    </div>
</div>
    ...
    <div class="col-12 col-lg-7">
        <div class="sb-panel">
            <div class="sb-panel-header">
                ...
                @if($isEdit)
                    <span style="font-size:12px;color:var(--sb-muted);">ID: #{{ $category->id }}</span>
                @endif
            </div>

            <form method="POST"
                  action="{{ $isEdit ? route('admin.staff-categories.update', $category) : route('admin.staff-categories.store') }}"
                  class="p-4">
                @csrf
                ...
                <div class="row g-3">
                    <div class="col-12">
                        <label class="form-label" style="font-size:13px;font-weight:600;">Category Name <span class="text-danger">*</span></label>
                        <input type="text" name="name" value="{{ old('name', $category?->name) }}"
                               class="form-control @error('name') is-invalid @enderror"
                               placeholder="e.g. Teaching Staff, Maintenance, Admin"
                    ...
                    <div class="col-12">
                        ...
                        <textarea name="description"
                                  ...
                                  style="font-size:13.5px;border-color:var(--sb-border);border-radius:8px;">{{ old('description', $category?->description) }}</textarea>
