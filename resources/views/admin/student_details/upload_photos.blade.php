...
<div class="d-flex align-items-center gap-3 mb-4">
    ...
    <div>
        <h5 class="fw-semibold mb-0" style="font-size:18px;">Upload Student Photos</h5>
...
<div class="sb-panel mb-4">
    ...
    <div id="tab-single" class="sb-tab-pane active p-4">
        <form action="{{ route('admin.student_details.upload-photos.submit') }}" method="POST" enctype="multipart/form-data" id="single-upload-form">
            @csrf
            <input type="hidden" name="upload_type" value="single">
            
            <div class="row g-4 mb-4">
                <div class="col-12 col-md-6">
                    <label class="form-label" style="font-size:13px;font-weight:600;">Select Student</label>
                    <select name="user_id" id="student_select" class="form-select" style="font-size:13.5px;border-color:var(--sb-border);border-radius:8px;" required>
                        ...
                        @foreach($students as $st)
                            <option value="{{ $st['id'] }}" data-has-image="{{ $st['has_image'] ? 'true' : 'false' }}">
                                {{ $st['name'] }} (Adm No: {{ $st['admission_number'] ?? 'N/A' }})
                            </option>
                        @endforeach
                    </select>
                </div>
    ...
    <div id="tab-bulk" class="sb-tab-pane p-4">
        <form action="{{ route('admin.student_details.upload-photos.submit') }}" method="POST" enctype="multipart/form-data">
            @csrf
            ...
            <div class="mb-4">
                <div style="font-size:13px;color:#0C4A6E;line-height:1.6;margin-bottom:15px;background:#F0F9FF;padding:15px;border-radius:8px;border:1px solid #BAE6FD;">
                    ...
                    The filename of each image MUST match the <b>Admission Number</b> of the student exactly (e.g., <code>ADM-001.jpg</code>).<br>
                    Images that do not match an existing admission number will be skipped.
                </div>
                ...
                <div id="zip-preview-container" class="mt-4" style="display:none;">
                    <div class="fw-bold mb-2" style="font-size:14px;">Preview: Students to be updated</div>
...
function previewZip(file) {
    ...
    fetch('{{ route("admin.student_details.upload-photos.preview") }}', {
        method: 'POST',
        body: formData
    })
    .then(res => res.json())
            ...
            if (data.matched.length > 0) {
                ...
                data.matched.forEach(item => {
                    ...
                    html += `<li class="list-group-item d-flex justify-content-between align-items-center">
                        <div>
                            <i class="bi bi-person me-2"></i> ${item.name} <span class="text-muted">(Adm: ${item.admission_number})</span>
                        </div>
                        ${badge}
                    </li>`;
                });
                ...
                html += '<div class="alert alert-warning py-2" style="font-size:13px;">No matching students found for the images in this ZIP.</div>';
            }
